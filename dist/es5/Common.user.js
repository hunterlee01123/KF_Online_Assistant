...
var handlePointsArea = function handlePointsArea() {
    ...
    var $changeCount = $points.find('[name="rvrc1"]').contents().eq(-3);
    var changeCountMatches = /当前修改配点可用\[(\d+)]次/.exec($changeCount.get(0).textContent);
    ...
    var countDownMatches = /\(下次修改配点还需\[(\d+)]分钟\)/.exec($points.text());
    if (countDownMatches) {
...
var getPromoteHaloInfo = exports.getPromoteHaloInfo = function getPromoteHaloInfo() {
    ...
    $.ajax({
        ...
        if (surplusMatches) {
            ...
            var _nextTime = Util.getDate('+' + (promoteHaloInterval - (_Const2.default.minPromoteHaloInterval - parseInt(surplusMatches[1]))) + 'm');
            Util.setCookie(_Const2.default.promoteHaloCookieName, _nextTime.getTime(), _nextTime);
            if (isInitLootPage) init();
            return;
        }
 ...
 * 在待检查的优秀帖页面上添加用户链接
 */
var addUserNameLinkInWaitCheckExcellentPostPage = exports.addUserNameLinkInWaitCheckExcellentPostPage = function addUserNameLinkInWaitCheckExcellentPostPage() {
 ...
 * 在已完成的优秀帖记录页面上添加版块链接
 */
var addForumLinkInCompleteExcellentPostPage = exports.addForumLinkInCompleteExcellentPostPage = function addForumLinkInCompleteExcellentPostPage() {
