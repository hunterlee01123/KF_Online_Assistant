...
(function () {
    ...
    if (action === 'modify') return;else if (options.attachType === 1 && (location.pathname === '/read.php' || action === 'reply' || action === 'quote')) return;else if (options.attachType === 2 && location.pathname === '/post.php' && !action) return;

    var _strlen = Info.w.strlen;
    ...
    if (options.showNoAttachOptionEnabled) {
        var switchHtml = '<input type="checkbox" name="noAttach" class="pd_input"> 不附加额外内容';
        if (location.pathname === '/post.php') $form.find('.pd_post_extra_option').append('<br><label>' + switchHtml + '</label>');else $form.find('input[name="Submit"]').after('<label style="margin-left: 7px;">' + switchHtml + '</label>');
    }
    $form.submit(function () {
        var $this = $(this);
        var $textArea = $this.find(location.pathname === '/post.php' ? '#textarea' : 'textarea:first');
        ...
        var getText = function getText(text) {
            if (typeof text === 'function') return Util.removeUnpairedBBCodeContent(handleText(text().toString()));else return Util.removeUnpairedBBCodeContent(handleText(text[Math.floor(Math.random() * text.length)]));
        };
