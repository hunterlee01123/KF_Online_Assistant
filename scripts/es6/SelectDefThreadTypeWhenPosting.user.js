...
(function () {
    ...
    const defThreadTypeList = {92: '完结动画', 68: '动漫音乐'};

    let fid = parseInt($('input[name="fid"]').val());
    if (!(fid in defThreadTypeList)) return;
    let $threadType = $('select[name="p_type"]');
    let threadTypeId = $threadType.find('option').filter((i, elem) => $(elem).text() === defThreadTypeList[fid]).attr('value');
    if (threadTypeId) {
