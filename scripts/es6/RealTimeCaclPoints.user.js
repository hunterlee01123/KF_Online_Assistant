...
let tempPlayerPropabilityCoefficient = 0.875;  // 冒险机制下调节默认事件发生概率的系数
let tempNPCPropabilityCoefficient = 1.33;    // 冒险机制下调节默认npc事件发生概率的系数
let lifePercent = 0.9;    // 当前生命值与下一层最大生命值之比的临界（为回血提供参考）
...
// 统计目前出现强化怪个数，需要用到统计npc的数组
    let num = 0;
    for (let i = 1; i <= enemyList.length - 1; i++) {
...
// 计算下一层出现强化怪概率，使用玩家预设值maxLevel、maxStrongNum直接计算
    ...
    let strongProbability = (maxStrongNum - totalStrongNum) / (maxLevel * 0.9 - currentLevel);
...
// 计算下一层出现强化怪概率，通过统计二项分布计算
    ...
    let riskingNextStrongNum = CritBinom(currentLevel + 1, strongProbability, riskingProbability); // 下一层出现强化怪的总个数大于该值的概率大于riskingProbability
